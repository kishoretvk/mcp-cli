...
class LLMProbe:
    ...
    def __init__(self, model_manager: ModelManager, suppress_logging: bool = True):  # ← CHANGED
        """
    ...
    async def test_provider(self, provider: str, test_message: str = "ping") -> ProbeResult:
        """
    ...
    def _is_valid_response(self, response: Any) -> bool:
        ...
        return (
            ...
            response["response"].strip() and
            not response["response"].strip().lower().startswith("error")
        )
